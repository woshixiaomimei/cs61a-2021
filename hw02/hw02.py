...
# two(f)(x)=f(one(f)(x))=f(f(zero(f)(x)))=f(f(x))

three = successor(two)
# three(f)(x)=f(two(f)(x))=f(f(one(f)(x)))=f(f(f(zero(f)(x))))=f(f(f(x)))
...
def pow_church(m, n):
    ...
    # I already have no idea about mathematics!
